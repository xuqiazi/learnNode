...
async function startHandle(ctx, next) {
  ...
  if (ctx.body && ctx.body instanceof stream.Stream) {
    ...
    try {
      ...
      body = ctx.body && ((typeof ctx.body === 'string' || ctx.body instanceof Buffer) ? ctx.body : JSON.stringify(ctx.body));
    } catch (e) {
      body = '';
      console.error(e);
    }
...
function loginController(ctx) {
  // console.log('requestBody',ctx.requestBody);
  ...
  const sessionId = `${Date.now()}${(Math.random() * 99999 + 1000)}`;
...
function getDataFromReq(req) {
  ...
  return new Promise(resolve =>
    ...
    if (contentType.startsWith('application/x-www-form-urlencoded')) {
      // form 表单提交
      console.log('HISIKY', querystring.parse(text));
